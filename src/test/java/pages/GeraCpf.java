...
public class GeraCpf {
    ...
    public void naoRepetirCPF(){
        String cpfConsultado;
        ArrayList cpfsCadastradosArray = RestAssured.given()
                .when().get("http://localhost:8080/api/v1/simulacoes")
                .then()
                .extract().path("cpf");
        String [] cpfsCadastrados = (String[]) cpfsCadastradosArray.toArray(new String[cpfsCadastradosArray.size()]);
        int i=0;
        while (i < cpfsCadastrados.length) {
            assertEquals(cpfsCadastrados[i], retorno);
            if (true) {
                cpfConsultado = cpf(false);
            } else {
                cpfConsultado = retorno;
            }
        }
    }
   ...
   private int randomiza(int n) {
    ...
    public static String main() {
        pages.GeraCpf gerador = new pages.GeraCpf();
